...
TAC *funcToEnd(TAC *pre, TAC *now, TAC *nex, FUNCTAG *fun); 
void printVar(SYMB *a, SYMB *b, ARMCODE *head);            
void printArr(SYMB *a, SYMB *b, ARMCODE *head);             
void printStr(SYMB *a, ARMCODE *head);                     
void cgHead();
SYMB *SYMBConst(int n); 
...
void cg(TAC* tacFirst)
{
    ...
    for (tacCode = tacFirst->next; tacCode != NULL; tacCode = tacCode->next)
    {
...
void cgPre(TAC *tacCode, FUNCTAG *fun, ARMCODE *head)
{
    ...
    while (nex != NULL)
    {
        ...
        {
        case 0:
            beginToFunc(pre, now, nex, chain);
            tag = 1;
            break;

        case 1:
            pre = funcToEnd(pre, now, nex, function); 
